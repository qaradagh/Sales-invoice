/* تبدیل تاریخ میلادی و شمسی (الگوریتم jalaali-js) */
(function (global) {
  'use strict';
  ...
  function isLeapJalaali(jy) { return jalCal(jy, true).leap === 0 ? true : jalCal(jy, false).leap === 0; }
  ...
  function weekdayName(jy, jm, jd) {
    try {
      return WEEKDAYS[toGregorian(jy, jm, jd).getDay()];
    } catch (e) {
      return '';
    }
  }
  ...
  function isValid(jy, jm, jd) {
    return jy >= 1200 && jy <= 1700 && jm >= 1 && jm <= 12 && jd >= 1 && jd <= monthLength(jy, jm);
  }
